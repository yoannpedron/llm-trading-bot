...
interface Props { src: string; title: string; autoPlay?: boolean; startAt?: number; onProgress?: (t: number, d: number) => void; onEnded?: () => void; onError?: () => void; muted?: boolean }

/** hls.js for .m3u8 (live), native <video> for progressive files. */
export default function Player({ src, title, autoPlay = true, startAt, onProgress, onEnded, onError, muted }: Props) {
  ...
  useEffect(() => {
    ...
    if (isHls && Hls.isSupported()) {
      ...
      hls.on(Hls.Events.ERROR, (_, d) => { if (d.fatal) { setError(`Flux indisponible (${d.type})`); onError?.() } })
    } else {
      video.src = src
    }
  ...
  }, [src, autoPlay, startAt, onProgress, onEnded, onError])

  return (
    <div className="relative aspect-video w-full overflow-hidden rounded-xl bg-black shadow-2xl">
      <video ref={ref} controls playsInline muted={muted} className="h-full w-full" title={title} />
      {error && (
