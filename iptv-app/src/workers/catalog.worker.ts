...
import { ColumnarBuilder, loadSnapshot, saveSnapshot, toWire, type Columnar, type Wire } from '../catalog/columnar'
...
export type WorkerOut =
  ...
  | { type: 'catalog'; col: Wire; source: 'snapshot' | 'server' | 'mock' | 'partial'; diff?: { added: number; removed: number } }
...
/** Send a catalogue to the UI: strings go as UTF-8 buffers, everything transferred, nothing copied. `col` is consumed. */
function send(col: Columnar, source: 'snapshot' | 'server' | 'mock' | 'partial', diff?: { added: number; removed: number }) {
  const { wire, transfer } = toWire(col)
  post({ type: 'catalog', col: wire, source, diff }, transfer)
}
...
self.onmessage = async (e: MessageEvent<WorkerIn>) => {
  ...
  try {
    ...
    const partial = () => { if (snap || partials >= 2) return; partials++; send(finish(b, cats, !!m.includeAdult), 'partial') }
    ...
    log('packed')
    // hand the catalogue to the UI first (strings are copied by postMessage, typed arrays are cloned cheaply here), persist afterwards
    const persist = { ...col, kinds: col.kinds.slice(), streamIds: col.streamIds.slice(), years: col.years.slice(), seasons: col.seasons.slice(), episodes: col.episodes.slice(), tmdbIds: col.tmdbIds.slice(), ratings: col.ratings.slice(), added: col.added.slice(), flags: col.flags.slice(), searchOffsets: col.searchOffsets?.slice() }
    send(col, 'server'); log('final posted')
    let diff: { added: number; removed: number } | undefined
    if (snapIds) { const now = new Set<string>(); for (let i = 0; i < persist.n; i++) now.add(persist.kinds[i] + ':' + persist.streamIds[i]); let added = 0; for (const k of now) if (!snapIds.has(k)) added++; let removed = 0; for (const k of snapIds) if (!now.has(k)) removed++; diff = { added, removed } }
    await saveSnapshot(m.key, persist).catch(() => undefined); log('snapshot saved')
